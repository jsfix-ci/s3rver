...
program
  ...
  .option('-a, --address <value>','Hostname or IP to bind to').preset(S3rver.defaultOptions.address)
  .option('-p, --port <n>','Port of the http server').preset(S3rver.defaultOptions.port)
  .option('-s, --silent','Suppress log messages').preset(S3rver.defaultOptions.silent)
  .option('--key <path>','Path to private key file for running with TLS').preset(fs.readFileSync)
  .option('--cert <path>','Path to certificate file for running with TLS').preset(fs.readFileSync)
  .option('--service-endpoint <address>','Overrides the AWS service root for subdomain-style access').preset(S3rver.defaultOptions.serviceEndpoint)
  .option(
  ...
  .option('--configure-bucket <name> [configs...]','Bucket name and configuration files for creating and configuring a bucket at startup').preset(parseConfigureBucket)
